import { RpgEvent, RpgMap, RpgShape, RpgWorld } from "@rpgjs/server";
import SAT, { Polygon } from 'sat';
import { TileInfo } from '@rpgjs/tiled/lib/classes/Map';
import { MobsOnMap } from "./utils";
...
export class Spawner {
    ...
    isValid = (polygon: Polygon, hitbox: Hitbox, x: number, y: number) => {
        ...
        // const hasCollisionWithPlayers = this.hasCollisionWithPlayers(x, y);
    ...
    hasCollisionWithPlayers = (x: number, y: number) => {
        const players = Object.entries(this.map.players);

        return players.some(([eventId, event]) => {
    ...
    populate = (spawnedMobs: MobsOnMap) => {
        ...
        console.log("spawnedMobs: ", spawnedMobs)
    ...
    populateSpawn = (spawn: Spawn, existingQuantity: number) => {
        ...
        // const players = RpgWorld.getPlayersOfMap(this.map.id)
